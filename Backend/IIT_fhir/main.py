...
def run():
    ...
    for file in tqdm(pdf_files, desc="Processing PDFs"):
        try:
            ...
            # Log raw LLM output for debugging financial data extraction
            raw_path = os.path.join("logs", file.replace(".pdf", "_raw_llm.json"))
            with open(raw_path, "w", encoding="utf-8") as rf:
                json.dump(data, rf, indent=2, ensure_ascii=False)
            logger.info(f"Raw LLM output saved: {raw_path}")

            # Log financial data summary
            benefits = data.get("benefits", [])
            n_with_limit = sum(1 for b in benefits if isinstance(b, dict) and b.get("limit_amount"))
            n_with_subs = sum(1 for b in benefits if isinstance(b, dict) and b.get("sub_limits"))
            logger.info(f"Financial summary: {len(benefits)} benefits, "
                        f"{n_with_limit} with limits, {n_with_subs} with sub-limits, "
                        f"sum_insured={data.get('sum_insured', '?')}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="NHCX PDF-to-FHIR Pipeline")
    parser.add_argument("--input", "-i", type=str, default=None,
                        help="Override input PDF folder (e.g. input_pdfs/test)")
